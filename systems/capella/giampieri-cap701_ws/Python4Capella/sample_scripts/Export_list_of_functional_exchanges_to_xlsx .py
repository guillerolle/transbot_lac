...
aird_path = '/01 - Giampieri/01 - Giampieri.aird'
...
for LC1 in all_LC:
    for LC2 in all_LC:
        ...
        list_FE = []
        for fe in all_FE:
            if fe.get_label() == 'energía de rotación en el eje der' and False:
                print(fe.get_label())
                print(fe.get_source_function().get_allocating_component().get_name())
                print(fe.get_target_function().get_allocating_component().get_name())
            #: :type LC1: FunctionalExchange
            """
            if fe.get_source_function() is not None:
                if fe.get_source_function().get_allocating_component() is not None:
                    if fe.get_source_function().get_allocating_component().get_name() == "Actuadores" and LC1.get_name() == 'Actuadores' and True:
                        print("LC1: " + str(LC1.get_name()))
                        print("fe..: " + str(fe.get_source_function().get_allocating_component().get_name()))
                        print("LC1: " + str(LC1.get_id()))
                        print("fe..: " + str(fe.get_source_function().get_allocating_component().get_id()))
                    if fe.get_source_function().get_allocating_component() == LC1:
                        print("CHECKING EQUALITY")
                # print(fe.get_source_function().get_allocating_component().get_name())
            """
            if (fe.get_source_function() is not None and fe.get_source_function().get_allocating_component() is not None):
                if fe.get_source_function().get_allocating_component().get_id() == LC1.get_id():
                    if (fe.get_target_function() is not None and fe.get_target_function().get_allocating_component() is not None):
                        if fe.get_target_function().get_allocating_component().get_id() == LC2.get_id():
