...
module Portico(width=500, height=1500, thickness=2, side=100){
    for (y=[1, -1]) {
        translate([0, y*(width/2-thickness/2), height/2-side/4])
        cube([side, thickness, height+side/2], center=true);
    }
...
module Unna(length=400, side=75, thickness=2){
    translate([0, 0, -thickness/2]){
        translate([thickness/2, 0, side])
        cube([thickness, side, side*2], center=true);
