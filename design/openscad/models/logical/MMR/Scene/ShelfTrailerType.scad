...
module Shelf_TrailerType(display="*", prefix="", length=500, width=500, height=500, tube_profile = [20, 20], sheet_thickness=3, castor_height=130, coupler_cube_size=100, coupler_position_z=250, coupler_position_x=200, coupler_hollow_size=80, coupler_fn=4)
{
            ...
            translate([0, 0, -castor_height]){
                ...
                translate([length/2+tube_profile[0]/2, 0, coupler_position_z+coupler_cube_size/2]){
                    ...
                    difference()
                    {
                        ...
                        translate([coupler_position_x, 0, 0])
                        rotate([0, 0, 45])
                        cylinder(h=coupler_cube_size*1.1, d1=coupler_hollow_size*1.1, d2=coupler_hollow_size*1.1*0.6, center=true, $fn=coupler_fn);
                    }
                }
            }
        }
...
width=500;
height=500;
coupler_cube_size=100;
coupler_position_z=325;
coupler_fn = 4;
Shelf_TrailerType(display=display, width=width, length=length, height=height, coupler_position_z=coupler_position_z, coupler_cube_size=coupler_cube_size, coupler_fn=coupler_fn);